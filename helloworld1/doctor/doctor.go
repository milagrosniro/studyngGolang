...
//mapea el array de strings y tranforma lo que esta en la primer columna, en lo q esta en la segunda columna
...
//array que adentro tiene otro array de obj?
...
func Response(userInput string) string {
	...
	//esto hace que seleccion un num random del array de arrays, cada array tiene muchas rtas posibles
	rand.Seed(time.Now().UnixNano())

	// Sanitize user input by setting up a regular expression to strip out punctuation
	//regex: para todo lo que no sea minuscula de a a la z o todo lo que no tenga mayuscla de la a-z y no tenga un digito del 0-9, que lo deje afuera
	...
	//lo que no sea letra o numero, que lo reemplace por un " "
